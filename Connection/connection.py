...
###############################################################
        ...
        #     Client_Public = RSA.importKey(Cpublic)
        #     encryptor = PKCS1_OAEP.new(Client_Public)
        #     Csend = encryptor.encrypt(Csend)
    
        #     conn.send(Csend + b':' + __enckey.encode())

        #     Sget = conn.recv(4096).decode().split(":")

        #     tmpsession = Sget[0]
        #     tmpre_auth = Sget[1]

        #     if(tmpsession==session):
        #         if(tmpre_auth==re_auth):

        #             return True, __enc.exportKey()
        ...
        # f = open(self.client_keys_path + "client_public.key", "r")
        # Cpublic = f.read().encode()
        # f.close()

        # f = open(self.client_keys_path + "client_private.key", "r")
        # Cprivate = f.read().encode()
        # f.close()
        
        # #ServerPublic
        # f = open(self.client_keys_path + "public.key", "r")
        # Spublic = f.read().encode()
        # f.close()

        # hashSpub = hashlib.md5(Spublic).hexdigest()

        # crendentials = Cpublic.decode() + ":" + hashSpub

        # s.send(crendentials.encode())

        # Cget = s.recv(4096)
        
        # __cget_array = Cget.split(b':')
        # Cget = __cget_array[0]

        # re_auth = __cget_array[1].decode()
        # __enckey = re_auth

        # CPub = RSA.importKey(Cpublic)
        # Cpri = RSA.importKey(Cprivate)

        # decryptor = PKCS1_OAEP.new(Cpri)
        # Cget = decryptor.decrypt(Cget).decode()

        # tmphashSpub = Cget.split(":")[0]
        # session = Cget.split(":")[1]

        # if(tmphashSpub==hashSpub):

        #     session = hashlib.md5(session.encode()).hexdigest()
        #     re_auth = hashlib.md5(re_auth.encode()).hexdigest()
            
        #     new_crend = session + ":" + re_auth

        #     s.send(new_crend.encode())

        #     return True, __enckey
    ...
    #     f = open(self.server_keys_path + "public.key", "w")
    #     f.write(public.decode())
    #     f.flush()
    #     f.close()

    #     f = open(self.server_keys_path + "private.key", "w")
    #     f.write(private.decode())
    #     f.flush()
    #     f.close()

    #     print(o.info_y("Server RSA Keys Generated "))
    ...
    #     f = open(self.client_keys_path + "client_public.key", "w")
    #     f.write(public.decode())
    #     f.flush()
    #     f.close()

    #     f = open(self.client_keys_path + "client_private.key", "w")
    #     f.write(private.decode())
    #     f.flush()
    #     f.close()

    #     print(o.info_y("Client RSA Keys Generated "))
